        ...
        <div className="max-w-7xl mx-auto">
            ...
            {plans.map((plans, index) => (
              <div
                ...
                className={`relative bg-slate-900/50 backdrop-blur-sm border rounded-xl sm-rounded-2xl p-6 sm:p-8 transition-all duration-300 overflow-visible group flex flex-col h-full${
                  plans.mostPopular
                    ? "border-blue-500 lg:scale-105 shadow-2xl shadow-blue-500/10"
                    : "border-slate-800 hover:border-slate-700"
                }`}
              >
                {/* Aninmation comes next */}
                ...
                <div>
                  <ul className="space-y-3 sm:space-y-4 sm:mb-8 mb-6 flex-row">
                    {plans.features.map((feature, index) => (
                      <li
                        key={index}
                        className="flex items-center space-x-2 sm:space-x-3"
                      >
                        <div className="flex-shrink-0 w-4 h-4 sm:w-5 sm:h-6 rounded-full bg-blue-500/20 flex items-center justify-center">
          ...
          <div className="mt-8 sm:mt-12 text-center">
            <p className="text-gray-400 text-base text-xl">
              Need a custome plan? <a href="#" className="text-blue-400 hove:text-blue-300">Contact our sales team</a>
            </p>
          </div>
        </div>
      </section>
